import { 
  BoltIcon, AcademicCapIcon, MapPinIcon, GlobeAltIcon, UsersIcon, 
  CheckCircleIcon, ArrowRightIcon, StarIcon, LightBulbIcon, PhoneIcon 
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 font-sans text-gray-800 tracking-tight overflow-x-hidden">
      ...
      {/* Developer Focus / New Batch Section */}
      <section id="developer-focus" className="py-20 bg-gradient-to-br from-purple-800 to-indigo-900 text-white shadow-2xl relative overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-10 flex items-center justify-center pointer-events-none">
            <StarIcon className="w-72 h-72 text-yellow-300 animate-pulse-slow" />
            <StarIcon className="w-52 h-52 text-blue-300 ml-20 animate-pulse-slow" />
        </div>
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <p className="text-xl md:text-2xl font-semibold text-pink-200 mb-8">
          ...
          <div className="bg-gradient-to-br from-indigo-700 to-purple-700 p-8 md:p-10 rounded-3xl shadow-glow border-b-8 border-red-500 transform hover:scale-[1.01] transition-all duration-300">
            <h4 className="text-2xl md:text-3xl font-extrabold mb-6 text-yellow-400">
              ✨ Essential Skills for Next-Gen Software Developers ✨
            </h4>
            <p className="text-lg md:text-xl mb-8 italic text-purple-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-left mb-10">
              {[
                { skill: "Python (Core Concepts)", desc: "Foundation for automation, data, and backend." },
                { skill: "Git & GitHub", desc: "Version control and collaboration essentials." },
                { skill: "Docker", desc: "Containerization for real-time deployment and DevOps." },
                { skill: "Java (Core & OOP)", desc: "Enterprise application development stronghold." },
              ].map((item, idx) => (
                <div key={idx} className="flex items-start space-x-3 text-lg md:text-xl text-white">
                  <CheckCircleIcon className="w-6 h-6 text-green-400 mt-1 flex-shrink-0" />
                  <p><strong>{item.skill}</strong> — {item.desc}</p>
                </div>
              ))}
            </div>

            <p className="text-2xl md:text-3xl font-extrabold text-red-400 mb-6">
              🚀 Learn. Build. Deploy. Grow your career with the right skills.
            </p>
            <a
              href="tel:9437401378"
              className="inline-flex items-center bg-red-500 text-white px-8 py-4 rounded-full font-black text-xl md:text-2xl shadow-neon hover:bg-red-600 transition-all transform hover:scale-105"
            >
